...
export interface TurnBridge { runTurn(options: RunTurnOptions): Promise<any>; }
...
export interface ClientToolPassthroughOptions { transport: Pick<BridgeTransport, "complete">; nativeTools?: ReadonlyArray<Record<string, unknown>>; onResponse?(event: { calls: Array<{ type: string; name: string; namespace?: string; callId: string }> }): void; }
export function createResponsesToolCodec(request: Record<string, any>, options?: Pick<ClientToolPassthroughOptions, "nativeTools">): { request: Record<string, any>; restore(response: Record<string, any>): Record<string, any> };
export function createClientToolPassthrough(options: ClientToolPassthroughOptions): { runTurn(options: RunTurnOptions): Promise<any> };
